...
def plot_interaction_network(ko_path, n_clusters=4, n_top_interactions=100, top_percent=50, output_file='interaction_network.pdf'):
    ...
    gene_total_strength = {gene: 0 for gene in gene_names}
    ...
    for ko_file in ko_files:
        ...
        for target_gene in gene_names:
            if target_gene in df_ko.index and target_gene != ko_gene:
                ...
                gene_total_strength[ko_gene] += strength
                gene_total_strength[target_gene] += strength
    ...
    for gene, count in gene_counter.most_common(5):
        print(f"{gene}: {count} times (total strength: {gene_total_strength[gene]:.2f})")
    ...
    gene_strengths = np.array([gene_total_strength[gene] for gene in gene_names])
    gene_strengths_sqrt = np.sqrt(gene_strengths)
    max_gene_sqrt, min_gene_sqrt = gene_strengths_sqrt.max(), gene_strengths_sqrt.min()
    node_sizes = 20 + (gene_strengths_sqrt - min_gene_sqrt) / (max_gene_sqrt - min_gene_sqrt) * 580
    ...
    sc.pl.umap(adata, color='Cluster', palette='tab20', frameon=False, 
               size=node_sizes, legend_loc=None, ax=ax, title='', show=False)
    ...
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', markerfacecolor='gray', markersize=8, label='Low'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor='gray', markersize=14, label='Medium'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor='gray', markersize=21, label='High'),
        Line2D([0], [0], color='gray', linewidth=2, label='Weak'),
        Line2D([0], [0], color='gray', linewidth=5, label='Strong')
    ]
    
    ax.legend(handles=legend_elements, loc='lower right', frameon=True, 
              facecolor='white', edgecolor='black', framealpha=0.9, fontsize=15)
...
plot_interaction_network("../results", n_clusters=4, n_top_interactions=100, top_percent=50, output_file='interaction_network_v3.pdf')